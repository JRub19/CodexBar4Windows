...
pub struct FactoryCredentials {
    /// Authorization: Bearer header value (without the `Bearer ` prefix).
...
impl Strategy for FactoryApiStrategy {
    ...
    async fn fetch(&self, _: &ProviderFetchContext) -> Result<UsageSnapshot, ProviderFetchError> {
        let creds = self.creds.resolve().await?;
        if !creds.has_auth() {
        ...
        let auth = self.fetch_auth_me(bearer, cookie).await?;
        let billing = self.fetch_billing_limits(bearer, cookie).await;
        let usage = self.fetch_usage(bearer, cookie, auth.user_profile.as_ref().and_then(|u| u.id.as_deref())).await?;
