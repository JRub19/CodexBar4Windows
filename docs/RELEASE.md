...
This runbook is the source of truth for shipping v1.0.4 and later. Stable
releases are blocked unless the Tauri minisign updater path is configured.
Authenticode signing is recommended, but not required for v1.0.4.
...
npm run tauri build
cd ..\..
pwsh scripts/build-portable.ps1 -Version 1.0.4
pwsh scripts/generate-checksums.ps1 -Version 1.0.4
```
...
```powershell
git tag -a v1.0.4 -m "CodexBar4Windows 1.0.4"
...
1. Download and install `CodexBar4Windows-1.0.4-x64.exe`.
2. Confirm the tray icon appears within a few seconds.
3. Open the popup and Preferences -> About; the version must read `1.0.4`.
